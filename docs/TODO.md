...
be restated whenever a pot moves without a row (Q48, schema v10). Later the
same day **accounts became plural, each in its own currency** (Q49, schema
v11): the switcher in Settings, transfers on `/tape`, per-currency `/mesic`
...
**The handoff's 2026-09-06 revision landed the same day** (`DECISIONS.md`, the
design revision): the dark theme lifted from black to charcoal; frosted glass
on what floats — the tab bar, the sheet, the toast — and on every control on
Zápis, which in the dark sits over a two-light glow; the selected segment is
the primary pill everywhere; the bar stands 24 px off the bottom; and
Nastavení became a hub of five rows with live summaries, each its own page
(`/nastaveni/ucty` … `/data`), which the rows on Já open directly. Glass stays
off anything inside a card — the handoff's own rule, against its prototype
(Q58) — and the currency beside the amount stays text (Q56).

**And an evening pass the same day** (`DECISIONS.md`, Q61): Já retired —
Nastavení is the last slot of the bar and Cíl and Jmění open from Domů's
cards; the three action pills and the sliders left Domů; the bucket search on
Zápis is pinned at the rail's right end; the account chips on Přehled are
glass, `.chip` now a primitive in `app.css`; and the status bar follows the
resolved ground on every theme, not the system scheme's literal. Then Q62: an
installed app looks for a new build on every resume and reconnect, announces
one in a toast with _Obnovit_, and reloads into it on the next navigation. And
Q63: a bucket under _Kam to šlo_ opens to its rows.
