...
| The goal is on the launch route, permanently: a card on Domů with this month's figure and its meter. | `/` |
...
**Evidence this law needed teeth:** nearly twice as much food was filed under
BYDLENÍ, LIFESTYLE, DARY and PROJEKTY across eight months as under JÍDLO itself.
...
- Tax reporting, invoicing, business accounting for a side business
...
| `refund-as-income` | An inflow that is really somebody paying you back            | "Bea - bydlení plyn" booked as income        |
...
Four of the workbook's eight months ran a deficit, and no screen he looked at
ever said so.
...
Five slots and two detail screens, since the third edition of the design
(2026-09-05). The bar reads **Domů · Výpis · ⊕ · Přehled · Nastavení**. The disc
in the middle opens `/zapis`, which carries no bar of its own — the keypad owns
the bottom of the phone there, and `✕` returns to whatever screen opened it.
`/cil` and `/jmeni` are reached from Domů as cards and keep that tab lit while
they are open; each wears a back chevron. Nastavení is the last slot itself
(Q61): the Já hub that stood in front of it until 2026-09-06 only repeated the
two cards Domů already carries. `/mesic` and `/platby` redirect into `/prehled`,
and `/ja` into `/nastaveni`, so a phone that still has them in its history lands
somewhere.
...
route could say how the month stands. The hub lasted a day: its two cards were
already on Domů, so the slot went to Nastavení (Q61).
...
│ └────────────────────────────────┘  │
│ ┌ Jmění celkem ─────────────── › ┐  │  → /jmeni
│ ┌ Poslední záznamy ─ Zobrazit vše┐  │  → /vypis · last three rows, every account
│ │ (◔) Bolt        LIFESTYLE · Včera │
├────────────────────────────────────┤
│ Domů Výpis (⊕) Přehled Nastavení   │
└────────────────────────────────────┘
```
...
┌────────────────────────────────────┐
│ (✕)       [ Výdaj │ Příjem ]       │
│ ┌ (Kč) Běžný účet · CZK  42 000,00┐│  ← the account rail (Q50): swipe = switch
│ └──────────────────────────────────┘│     · · ·
│                Výdaj               │
...
the new bucket's name, the editor opens over the list, and the bucket it
commits is selected for the entry being typed (Q65).

On save the screen goes home and the toast says what was written — _−410,00 Kč
· JÍDLO · Restaurace_ — with Zpět on it. The toast's glass is tinted the
direction of what moved, red for an expense and green for an income (Q65).
The date survives the save, so several rows for one back-filled day are typed
...
check. Tap a row to edit or delete; the edit sheet's category field opens the
same list Zápis does, with the same row for a bucket that does not exist yet.
The address was `/tape` until 2026-09-07; the old one redirects (Q65).
...
A hub of five rows — a circle, a title, a live one-line summary, a chevron —
each opening its own page with a back chevron to the hub: `/nastaveni/ucty`,
...
(`docs/redesign/third-edition-handoff/README.md`) and applied as
specified: a consumer-banking layout in the manner of a modern neobank. A soft-grey
...
**Light first.** The default is the light theme; dark — a charcoal ground with
graphite cards — is the system preference or the explicit choice. Both are
...
**What floats is glass.** The tab bar, the sheet and the toast, the round
buttons, and every control on Zápis — which in the dark sits over a glow — are
frosted; nothing inside a card is, because a card has nothing to frost
(`DECISIONS.md`, Q58).
...
│  │  ├─ src/routes/                # / · /zapis · /vypis · /prehled · /cil
│  │  │                             #   · /jmeni · /nastaveni and its five pages
...
| Sync merge corrupts the ledger                              | Low               | Soft delete + append-mostly + idempotency. Nightly `pg_dump` to off-box storage.                                        |
