# Handoff: Prosper — third-edition redesign

## Overview

A full visual and structural redesign of Prosper (SvelteKit 2 / Svelte 5 PWA, Czech UI, Dexie, 150 kB brotli budget). The current "graphite instrument" design — true-black, keypad-first, four-tab bar — is replaced by a consumer-banking layout: soft-grey ground with white cards (dark counterpart: charcoal `#121416` ground with `#1f2226` cards), one proportional sans, pill buttons and chips, circular category icons, a five-slot tab bar with a central record disc, and a home screen that leads with the month's net figure.
...
| Role | Light | Dark |
| --- | --- | --- |
| ground `--ground` | `#f4f4f4` | `#121416` |
| card `--surface` | `#ffffff` | `#1f2226` |
| soft (chips, tracks, secondary pills) `--surface-3` | `#f0f0f2` | `rgba(255,255,255,.08)` |
| well (recessed slab, e.g. goal "why") `--ground-2` | `#f4f4f4` | `#191c1f` |
| glass `--glass` | `rgba(255,255,255,.72)` | `rgba(31,34,38,.72)` |
| glass edge `--glass-edge` | `rgba(255,255,255,.6)` | `rgba(255,255,255,.1)` |
| glass shadow | `0 8px 32px rgba(25,28,31,.14)` | `0 8px 32px rgba(0,0,0,.45)` |
| Zápis glow `--glow` (dark only) | none | `radial-gradient(60% 34% at 20% 8%, rgba(73,79,223,.45), transparent 70%), radial-gradient(50% 30% at 90% 42%, rgba(0,168,126,.28), transparent 70%)` |
| ink `--ink` | `#191c1f` | `#ffffff` |
| mute `--ink-2` | `#505a63` | `rgba(255,255,255,.74)` |
| stone `--ink-3` | `#8d969e` | `rgba(255,255,255,.52)` |
...
No shadows on resting surfaces. Shadow only on floating glass layers (tab bar, toast: glass shadow above) and the bottom sheet (`0 -12px 48px rgba(0,0,0,.25)`).

### Glass (revision 2026-09-06)

A frosted surface: `background: var(--glass); border: 1px solid var(--glass-edge); backdrop-filter: blur(20–24px) saturate(1.6)` (+ `-webkit-` prefix). Applied to layers and controls that sit **over** content — never to cards or page grounds:

- **Tab bar** — floating pill, `left/right: 16px; bottom: 24px; height: 62px; border-radius: 9999px`, blur 24, glass shadow. Pages end with 120px bottom padding so the last row scrolls clear.
- **Bottom sheet** (category editor) — glass bg + top edge, blur 28, existing sheet shadow.
- **Toast** — glass, ink-coloured text, 100px above the bottom.
- **Controls**: 40px round icon buttons (back, ✕, settings), secondary/soft pill buttons, small tag pills, the theme segmented track, and on Zápis the account cards, direction segment, currency pill, date/payee fields and the check strip.
- Selected segment in any segmented control = primary pill colours (`--pill` / `--pill-ink`), so it reads against glass.

### Zápis in dark mode

The screen carries `background-image: var(--glow)` — two soft radial lights (indigo top-left, teal mid-right) behind the glass surfaces. Light theme: `--glow: none`. Keys stay bare (transparent, active = soft), Uložit is the primary pill.
...
2. **Hero** (centered): label "Zůstatek měsíce" 13 mute; net figure 44/600 signed ("+3 500,00 Kč"); two pills below on card bg, 13/600: "↑ 50 000,00" in colour `--in`, "↓ 46 500,00" in mute.
...
3. **Month row**: "Srpen 2026" 17/600 left; two small pills right (12/600, card bg): "↓ 46 500,00" mute, "↑ 50 000,00" `--in`. Collapsible per month as today.
...
### Nastavení (`/nastaveni`) — hub + five detail pages (revision 2026-09-06)

Back header "Nastavení". One card, rows 64px with hairlines: 40px glass circle with icon (wallet, tag, sun-moon, refresh-cw, database), title 15/600, one-line live summary 13 mute (e.g. "Běžný účet · 42 000,00 Kč", "10 kategorií, ikony a barvy", "tmavý", "zatím jen tenhle prohlížeč", "128 záznamů · záloha a export"), chevron. Each row opens its own page (`/nastaveni/ucty`, `/kategorie`, `/vzhled`, `/sync`, `/data`) with a back chevron to the hub. The Nastavení rows on **Já** deep-link straight to the matching page. State: `setPage: null | 'ucty' | 'kategorie' | 'vzhled' | 'sync' | 'data'`, reset on any tab change.

Page contents, one card each: **Účty** (account row with 40px accent circle, name, "běžný · zapisuje se sem", balance; pills "Přidat účet" / "Upravit"), **Kategorie** (rows 52px: 34px icon circle, name 15, type badge, chevron — **tap opens the category editor sheet**; hint "Ťukni na kategorii a vyber jí ikonu a barvu."; "Nová kategorie" pill), **Vzhled** (3-segment pill on soft track: systém / světlý / tmavý; active = card bg), **Synchronizace** (copy + "Spárovat" primary 44px), **Data** (three soft pills, facts rows, "Začít znovu" as a 48px pill outlined 1.5px danger, danger text).
...
- **2026-09-05** — initial handoff.
- **2026-09-06** — dark theme lifted from black to charcoal; frosted-glass floating tab bar, sheet, toast and controls; Zápis dark-mode glow; Nastavení split into a hub and five pages.

## Files

- `Prosper.dc.html` — the clickable prototype (all screens, both themes via the `theme` prop / Vzhled segment)
