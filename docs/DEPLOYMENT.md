...
The phone picks up a new client by itself (Q62): the app asks for a new
service worker every time it comes back into view or back online, the worker
installs the new shell and takes over, and the screen still running the old
bundle shows _Nová verze je připravená · Obnovit_ and reloads on its own at the
next navigation. Bring the app to the front, or tap Obnovit, and the new build
is there — no visit to the browser and no manual reload. If it seems stuck on
the old one, that is a cached `200.html`, and the container's nginx serves HTML
as `no-cache` precisely so it cannot be.

**A manifest change is slower than a build.** The installed app is a WebAPK
whose display mode, icons and colours were baked in at install; Chrome
