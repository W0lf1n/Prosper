...
- **`Výdaje 2026.xlsx`** is a real file and the source for every
...
anything, and nearly twice as much food as JÍDLO reported was invisible.
...
`prosper.example.com` is what the runbook uses as its placeholder. Nothing in
the repository hard-codes a domain: it is typed
...
| ----------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| Food filed under BYDLENÍ, LIFESTYLE, DARY and PROJEKTY                                    | JÍDLO reported a third of what was actually spent on food                                            | `misfiled` — matches the description against the vocabulary and offers the right bucket in one tap |
| OSTATNÍ as a dumping ground: a five-figure "opak. obj", a four-figure "objednávka"         | Roughly a sixth of eight months' outflow — the second largest bucket, explaining nothing             | `other-overflow` and `vague`                                                                       |
| "Netflix - 379" recorded as 74 Kč; "objednávka (2415 Bea)" as 5 008 Kč                    | Shares of joint payments, with no record of who owes the rest                                        | `unclear-number`                                                                                   |
| A five-figure front door in the same column as the groceries                              | KVĚTEN cost sixty percent more than a usual month, and dragged the yearly average with it            | `one-off` plus a separate "běžný chod měsíce" figure                                               |
| "Bea - bydlení plyn 1 250" booked as income while the full 2 500 Kč sat in BYDLENÍ        | Income and spending both inflated; the savings rate is meaningless                                   | `refund-as-income`                                                                                 |
...
| `E2`, labelled "CELKEM ZA 12 MĚSÍCŮ", is `F5-E5`                                          | It is the average monthly surplus, and it is negative                                                | `/mesic` states income, outflow and net separately, per month                                      |
...
| 102 descriptions with trailing spaces; a dozen subscriptions retyped monthly              | Fragmented grouping; the −18 Kč error survived two months                                            | Trimmed on save, autocompleted from history, `duplicate` check                                     |

Four of the eight months ran a deficit, and the spreadsheet never said so on any
screen he looked at. `/mesic` does, per month, in the `CELKEM` figure.
...
"Dlouhodobá investice", the same sum every month without fail, sat inside OSTATNÍ.
The one number the Targeting law is built on was buried in the dumping ground.
It is now its own bucket. Životní pojištění and one other fixed premium, eight
months each, are still in OSTATNÍ and are the next two candidates.
...
to be in SPOŘENÍ beforehand. "Money in the
...
A flat worth millions makes `celkem` a number that is true and useless — every
...
A dozen subscriptions were retyped by hand every month in the workbook, and a
...
- **An amount override never rewrites the schedule.** The gas bill is 2 000 Kč
  most months and 3 000 Kč in February.
...
untouched: 30 000 Kč leaves the account, because 30 000 Kč leaves the account.
...
is the net year — 180 000 Kč, not 360 000 Kč. Both are true; only one of them is
...
is that it was the *only* place: the tape row is where the app tells you Alex
owes you 420, so the tape row is what gets tapped when Alex pays. Opening it
...
Asked for: a different background per currency, as banking apps do, so a foreign
...
investing account I am not sending any money to, a round six-figure sum. This record I am
...
into the one CZK account — show what that sum is made of: _"Total 25 000 Kč —
Běžný účet 20 000 Kč — Revolut 5 000 Kč. And also for other currencies."_
...
## Design pass · third edition — the consumer-banking layout · 2026-09-05
...
`docs/redesign/third-edition-handoff/`.
The brief: replace the graphite instrument with a consumer-banking layout in the
manner of a modern neobank, and change nothing about flow, domain, checks, sync or the
...
## Design revision · 2026-09-06 — charcoal, glass, the glow, and a settings hub

Petr revised the handoff a day after the third edition landed — the README's
"Revision history" entry for 2026-09-06, and a second sheet, `Zapis options.dc.html`,
with three ways to lift the record screen in the dark, of which the second,
**Glass on glow**, is the one the README then specifies. Four changes, applied
as specified, and two places where the implementation had to read the handoff
rather than copy it.

### The dark theme lifted from black to charcoal

Ground `#000000` → `#121416`, cards `#16181a` → `#1f2226`, the well `#0a0a0a`
→ `#191c1f`, mute and stone up two points (white at 74 % and 52 %). The reason
is the glass: a frosted surface needs a tone to frost against, and a card has
to be a step lighter than what it sits on — a 72 % charcoal over true black is
black. The pressed card, `--dark-surface-2`, moved with it to `#272b30`, the
value the option sheets use for their raised controls. `theme-color` in
`app.html` follows, being the one colour written twice (rule 9). The currency
tint (Q50) still leaves the dark ground alone: it is charcoal in every currency
now, as it was black.

### Q58 — Glass is for what sits over content, and a card is not content · answered 2026-09-06

The handoff states the rule itself: glass is *"applied to layers and controls
that sit over content — never to cards or page grounds."* The prototype then
paints every soft pill glass, including the ones inside white cards, where in
the light theme `rgba(255,255,255,.72)` over `#ffffff` under a white edge is
invisible, and in the dark `rgba(31,34,38,.72)` over `#1f2226` is the card
itself with a 10 % edge around it. That is a prototype's shortcut, not a
drawing, so the rule was applied and the shortcut was not:

- **Glass**: the three floating layers — the tab bar, the sheet, the toast —
  the 40 px round buttons, and every control on Zápis (the account cards, the
  direction segment, the bucket chips, the date, the payee, the check strip
  and its expanded rows). Each sits on the ground or over content.
- **Not glass**: a pill, a badge or a segmented track *inside a card*. It stays
  the soft surface, because a card has nothing behind it to blur. The hub's
  five icon circles are the one thing the handoff draws as glass inside a
  card; they are soft, with the icon in the ink.

One primitive — `.glass` in `app.css` — and two that are glass by declaration,
`.round` and `.seg--glass`. The floating layers restate the recipe in their own
components because their blur radii differ (24 for the bar and the toast, 28
for the sheet), and each carries its own opaque fallback under
`@supports not (backdrop-filter: …)`, since translucent-without-blur over
scrolling text is worse than a card. Recorded so the next reading of the
prototype does not "fix" the pills inside cards back to invisible.

### The selected segment is the primary pill, everywhere

`.seg--soft` used to select with the card colour. Every segmented control now
selects in `--pill` on `--pill-ink`, so the choice reads against a glass track
and a soft one alike — the handoff's line, and one rule fewer.

### The toast went glass, in the ink

It was the primary pill's colours. It is now glass with ink text and the glass
shadow, 16 px above the bar; Zpět on it becomes the small primary pill, being
the one action in the strip. `--elev-toast` is gone. `--elev-glass` — the
handoff's "glass shadow" — replaces `--elev-bar` and serves both the bar and the
toast; the sheet keeps `--elev-sheet`.

### Zápis in the dark carries the glow

`--glow` — two radial lights, indigo top-left and teal mid-right, the accent
and the money-in hue at the strength of a lamp in the next room — is the record
screen's `background-image`, and `none` in the light theme. The keys stay bare:
transparent, and the soft surface when pressed, where they used to light up to
the card colour. **Q56 stands**: the currency beside the amount is text. The
revised handoff draws it as a glass pill with a chevron, but the account rail
is the switch, and frosting the second control does not make it less of a
second control.

### The bar stands 24 px off the bottom

It was 12. `--tabbar-lift` is `max(24px, 12px + safe-area-inset-bottom)`: 24 on
a phone with no home indicator, 12 past the indicator on one that has it,
rather than 24 stacked on top of it. The page ends bar + lift + 32 px under
that (the handoff's 120), the toast 16 above the bar (the handoff's 100).

### Q59 — Nastavení is a hub and five pages · answered 2026-09-06

`/nastaveni` is five rows — a 40 px circle, a title, a live one-line summary, a
chevron — each opening its own route: `/nastaveni/ucty`, `/kategorie`,
`/vzhled`, `/sync`, `/data`, with a back chevron to the hub; the hub's chevron
goes to Já. Já's five rows deep-link straight to the pages and show the same
summaries: `lib/ui/settings.ts` → `settingsRows()` builds the list for both
screens, so the two cannot drift. The summaries are the active account and what
is on it; the count of live buckets ("11 kategorií, ikony a barvy" —
`CATEGORIES` joined `czech.ts`); the theme, "podle systému" when nothing is
chosen; the sync state, "zatím jen tenhle prohlížeč" while unpaired
(`SYNC_LABEL` moved into `sync/status.svelte.ts`, one dictionary in place of
the two that had disagreed about the word for *off*); and the row count with
"záloha a export".

The 1 180-line Settings page split five ways with nothing changed inside a
section — each page kept its script, markup and scoped styles, and dropped the
card label, since the title is in the bar. The anchor-scroll effect Já used to
reach a section went with the sections; the `also` list in `TabBar.svelte`
matches by prefix now, so the five pages keep Já lit. Three icons joined
`Icon.svelte` for the hub — sun-moon, refresh-cw, database — as chrome, not
category glyphs, so the editor's grid (`CATEGORY_ICONS`) does not offer them.
And the spend-type dictionary, written three times over (the picker, the
editor, Settings), is `SPEND_TYPE_LABEL` in `domain/prosperity.ts` once.

### Q60 — The tab bar's glass is liquid, not frosted · answered 2026-09-06

Petr asked for the *liquid* glass — the lens that bends what is under it —
rather than the static frost, starting with the bar. The handoff has no such
thing: its glass is a fill, a 1 px edge, `blur(24px) saturate(1.6)` and a
shadow, and that is what the bar had. The bar alone is liquid now, in the
handoff's tokens and in four parts:

- **The bend.** An SVG filter, `#glass-refract`, runs in the backdrop ahead
  of the blur: `feImage` carries a displacement map drawn on a canvas for the
  bar's exact size — red the horizontal push, green the vertical, zero along
  the pill's axis and rising as the cube of the distance to the rim, pointing
  outward — and `feDisplacementMap` bends the backdrop by up to 32 px with
  it. The middle of the pill is a window; the rim shows the content just
  outside it, compressed, the way a thick lens does. A `ResizeObserver`
  redraws the map when the bar's width changes; nothing else is scripted.
- **The frost thins.** A 24 px blur smears the bend to nothing — under the
  handoff's frost the bar looked static because it was — so the liquid bar
  blurs 6 px, saturates 1.8, and its fill is the glass at 82 %, enough for
  the bent content to show. This is the one place the handoff's blur is not
  the blur.
- **The rim and the thickness.** `::before`, a 1 px ring masked out of a 135°
  gradient — `--glass-shine` where the light comes from at the top left,
  `--glass-edge` on the way round, `--glass-rim` in shade at the bottom right,
  lit again at the far corner; a border is one colour, a ring can turn. And
  `::after`, the shine entering along the inside of the top edge and gone by
  the middle, the way light enters thick glass.
- **The lens.** `--glass-lens`, the glass a step lighter with a rim of its
  own lit from above, in the slot of the current tab, and it *springs* to the
  one chosen — `--ease-spring` overshoots and comes back, the way liquid
  settles. It is the only moving part; `prefers-reduced-motion` stops it like
  everything else.

Three colour tokens and one easing joined `tokens.css`. Nothing scales and
there is no inset shadow — the rings are gradients under a mask (rule 16).

**The bend is Chromium's alone, by construction.** WebKit and Gecko do not
apply an SVG filter to a backdrop, and a declaration they half-parse could
cost them the blur, so the `url()` filter is only written where
`navigator.userAgentData` exists — a Chromium engine, the one that renders it
— and Safari, iPhone Safari included, gets the frost, the rim, the thickness
and the springing lens without the bend. The map is a few kilobytes of PNG in
a data URL, built once per size; the filter costs Chromium a displaced,
lightly blurred 62 px strip per frame while the page scrolls, which a phone
carries. The other glass surfaces — the sheet, the toast, the Zápis controls
— keep the handoff's frost until this one has been lived with.

---

## The 2026-09-06 evening pass — Já retired, and the status bar follows the ground

Petr's list after a day with the revision on the phone: five items, four of
them things the third edition had put on the screen twice.

### Q61 — Já is gone: Nastavení is the last slot, and Domů carries nothing the bar already does · answered 2026-09-06

**Question.** The third edition's Já hub was three cards — Cíl, Jmění,
Nastavení — and Domů already carried the first two, with the settings behind a
sliders button in its corner and three action pills (Zapsat · Výpis · Měsíc)
under the net figure, every one of which is a slot in the bar below. Petr: the
pill with Cíl and Jmění is already on the home screen, so Já is not needed —
make the last slot Nastavení itself, and take the button and the three pills
off Domů.

**Answer.** Done as asked. The bar reads Domů · Výpis · ⊕ · Přehled ·
Nastavení, with the sliders icon in the last slot. `/ja` is a redirect stub to
`/nastaveni`, the way `/mesic` and `/platby` are to `/prehled`: an installed
app can reopen on its last address, and two days is long enough for that.
`/cil` and `/jmeni` keep **Domů** lit now, since that is where their cards
are, and `AppBar`'s chevron defaults to `/`. `/nastaveni` is a tab screen: its
title sits in the flow like every other tab's, and the back chevron went with
the hub it pointed at. Domů's header is the month alone, and the hero's two
pills are followed by the cards directly. `settingsRows()` in
`lib/ui/settings.ts` has one caller now and stays where it is; the `person`
icon left `Icon.svelte` with its only user.

Nothing the four laws name moved: recording is still one tap from anywhere —
the disc — and the month's net still leads the launch route.

### The status bar follows the resolved ground, not the system scheme

The first screenshot: a pale strip across the top of Zápis on the phone, where
the bottom edge had gone edge-to-edge. The strip was the Android status bar,
painted from `theme-color` — and `app.html` carries two of those, one per
`prefers-color-scheme`, which the browser chooses between by the *system*
scheme. With the phone in light mode and the app set to dark in Vzhled, the
light literal won: `#f4f4f4` over charcoal. `syncThemeColor()` in `tint.ts`
already pointed both metas at the resolved `--ground` — but only while a
currency tint was on, restoring the literals the moment it went, which for the
home currency is always.

Now it writes the resolved ground unconditionally, on every theme change and
every tint change, and the literals in `app.html` are for the first paint
only. That first paint is covered too: the inline script that stamps
`data-theme` before the stylesheet arrives also copies the chosen scheme's
literal into both metas, so a dark launch on a light phone never shows the
light strip at all. No third copy of the colour — the script reads the literal
off the meta that has it. The manifest's `theme_color` stays `#f4f4f4`: it
paints the frame before the document exists, can carry only one value, and
the meta overrides it the moment the page loads.

Over Zápis's glow the bar is the flat ground against a lit corner. That seam
is the cost of a status bar that cannot be transparent on Android, and it is a
hairline next to a pale band.

### The bucket search is pinned at the rail's end

The magnifier that opens the whole category list was the last pill of the
rail, which on a ledger of eleven buckets meant two screens of scrolling to
reach it — "not handy". `CategoryPicker` is now a row of two: the rail,
scrolling, and the search pill after it, outside the scroll, so it is under
the thumb whatever the rail shows. The rail's own right gutter is a mask fade,
so a pill leaving under the magnifier dissolves rather than being cut mid-word,
and a rail scrolled to its end still shows its last pill whole.

### The account chips on Přehled are glass, and the chip is a primitive

The second screenshot: the account switcher in the dark — Běžný účet · Revolut
· vše — unreadable. Two faults. The pills were `.btn--card`, graphite on
charcoal, a step too close to the ground for text at 13 px. And the chosen one
carried `.btn--primary` *and* `.btn--card`, in that order in `app.css`, so the
card fill outranked the pill fill while the ink stayed the pill's black: black
on graphite. Both go the same way: the switcher is the bucket rail's chip —
glass on the ground, the chosen one the primary pill, like a segment (rule 16:
rank is fill). The recipe moved from `CategoryPicker.svelte` into `app.css` as
`.chip`, glass by definition beside `.round` and `.seg--glass` (rule 8: one
definition, and a screen declares only the difference), and Přehled declares
exactly that — even padding, having no circle. `.btn--card` and
`.btn--primary` are no longer combined anywhere; a chosen pill on the ground
is a chip.

### Q62 — A new build reaches the installed app on its own · answered 2026-09-06

**Question.** After a deploy the phone stayed on the old version until Petr
opened the address in the real browser, reloaded there, and came back to the
installed app. Is there a better way?

**Answer.** The worker was never the problem. `service-worker.ts` precaches
the shell under a per-build cache name, `skipWaiting()`s at install and
`clients.claim()`s at activate, so a new worker takes over the moment the
browser *finds* it. The browser finds it on a navigation into the worker's
scope, on `register()`, and at most once a day otherwise — and an installed
app brought back from the background does none of those. It resumes. A phone
that keeps Prosper in its recents never asks, and the trip to the browser was
the navigation that did.

So the app asks. `lib/ui/update.ts` → `watchUpdates()`, mounted in the layout,
calls `registration.update()` on every `visibilitychange` to visible and every
`online` — one conditional request for `service-worker.js`, byte-compared by
the browser, nothing installed unless it changed; nginx already serves it
`no-cache`, and the 24-hour rule bypasses the HTTP cache for a worker script
regardless. When a new worker has taken over, `controllerchange` fires in the
page that is still running the old bundle; that page shows _Nová verze je
připravená_ with **Obnovit** and, whether or not the pill is tapped, reloads
itself on the next navigation (`afterNavigate` → `applyUpdate()`). The first
install fires `controllerchange` too — uncontrolled to controlled — and is
ignored, since there is no older bundle to leave.

**Why the next navigation and not at once.** A reload under a half-typed
amount is the one thing this app must never do (rule 5's spirit: the network
never gets in the way of the keypad). A navigation is the moment nothing is
half-typed; it is also the moment the old bundle would otherwise ask the cache
for a route chunk the new worker has already deleted, which SvelteKit answers
with a hard reload of its own. Doing it deliberately is the same reload with
the reason on screen first.

The toast learnt an `action` for this — a label and a callback — and `undo`
became the common case of one, labelled Zpět; nothing that shows a toast had
to change. The pill in `Toaster.svelte` is `.toast__action` now.

### Q63 — A bucket under Kam to šlo opens to its rows · answered 2026-09-06

**Question.** The ranking says ZDRAVÍ took a four-figure sum in two rows. Which two,
and when? Petr wanted to see each item in a bucket without leaving the screen
— "like an accordion".

**Answer.** `BucketTotal` carries `rows` now — the bucket's own rows, newest
first, in the tape's order (the day, then the moment written) — computed in
`summariseMonth()` beside the total, since the grouping is already there, and
tested. On Přehled the whole bucket line is a button: it presses by luminance
like a row, carries `aria-expanded` and a chevron that turns, and opens to
the rows as `.row` primitives — who or what, the day, one-off marked, the
signed amount with a refund in green. One bucket at a time, so the list stays
a ranking; the open one is keyed with the month and the account, so turning
the page or switching the account closes it by construction, with no effect
to reset. A row without a payee or a note reads "bez popisu".

Nothing on the row is editable here: the tape is where a row is edited, and
Přehled is a report. That is the same line the receivables kept (§8, "Dluží
mi is on the row").

### Q64 — The installed app is fullscreen: no status bar at all · answered 2026-09-06

**Question.** With Q61's fix the status bar should have been the charcoal
ground, and on the phone it was still a pale strip — and the ask changed
shape: "can the app go all the way to the top of the screen, like a
fullscreen app?"

**Answer.** Yes, and that is now what the installed app does. The manifest
carries `"display_override": ["fullscreen"]` above `"display": "standalone"`:
an installing browser that knows the override runs the app with no status
bar — the page owns the whole panel, top to bottom — and one that does not
falls back to standalone as before. `viewport-fit=cover` was already there,
so under a camera cutout the page keeps its `env(safe-area-inset-top)`
padding rather than putting the month under the lens.

**The cost, stated.** In fullscreen the clock, the battery and the
notification icons are hidden while Prosper is in front; a swipe from the top
edge shows them. For an app that is opened for five seconds to type a number
that is a fair trade, and it is what Petr asked for. The `theme-color` work
from Q61 stays: it still paints the status bar of a browser tab, the task
switcher's card, and the standalone fallback.

**Why the strip was still pale is not settled.** Either the phone was still
on the build before Q61 — the update path of Q62 did not exist yet, and a
WebAPK resumed from recents never asked for a new worker — or the WebAPK
paints its bar from the manifest's baked `theme_color` and ignores the page's
meta. Fullscreen makes the question moot, which is the point of choosing it
over another round of guessing at a device that is not on the desk. If the
answer ever matters again, the test is a fresh install after a deploy, in
standalone, with the phone in light mode and the app in dark.

## The 2026-09-07 pass — Výpis's address, a bucket made mid-entry, and a toast that knows its direction

### Q65 — `/vypis`, a bucket from the picker, and a tinted toast · answered 2026-09-07

**Question.** Four asks in one message. Výpis should live at `/vypis`, not
`/tape`. The edit sheet on Výpis still showed a native `<select>` for the
category where Zápis has a searchable list of circles. A bucket that does not
exist yet should be creatable from that list, and the entry being typed should
go to it. And the money toast should stay glass but be red for an expense and
green for an income.

**The address.** The route directory is `routes/vypis`; `routes/tape/+page.ts`
redirects to it, like `/mesic`, `/platby` and `/ja` do, because a phone may
have the old address in its history or on its home screen. The tab bar lists
`/tape` under Výpis's `also`, so the redirect frame is not a moment with no
tab lit. The code name — `buildTape`, the `tape` icon — stays: it is the word
for the ledger, not the address.

**One list.** The picker sheet Zápis opened from its search pill is now its own
component, `CategoryPickerSheet`, and the edit sheet on Výpis opens the same
one from a category field that reads like a row — circle, name, chevron. Its
list is the row's direction, ranked by use on the account, and always includes
the row's own bucket even when that is archived or on the other side, so what
a row has is never hidden from it. The `<select>` is gone.

**A bucket made mid-entry.** The list ends in a row: _Nová kategorie_, or
_Přidat kategorii „Kino“_ once something is typed that no bucket already is.
It opens the category editor over the list, pre-named and on the entry's
direction — a bucket made from Příjem is an income bucket. The editor commits
through the page, which writes it with `createCategory` and selects it, and
both sheets close: the entry being typed goes to the bucket that was just
made. The rail scrolls the new pill into view rather than to its start, which
is the one change to `CategoryPicker`'s reveal logic. `lib/ui` still does not
import the repo; the sheet emits and the page writes, as Settings always did.

**The toast.** Two glass tokens, `--glass-out` and `--glass-in`, red and green
at 82 % with a white line and a lighter edge, theme-independent like the
category palette. This is the one place money going out has a hue, and the
hue is the danger red, on purpose: a toast is the moment a mis-tap is still
