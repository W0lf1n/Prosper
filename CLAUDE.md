...
**Last revised:** 2026-09-16 · schema v15 · 543 web tests · 56 API tests
...
16. **Elevation is luminance; press is luminance; every button is a pill.** A
    ...
    shadow; real shadow survives only under what genuinely floats — the glass
    tab bar, the glass toast, the sheet. **Glass (`.glass`) is for what sits
    over content or on the ground** — those three, the round buttons, every
    control on Zápis — and never for anything inside a card, which has nothing
    to frost (Q58). Inside a card, recessed is `--ground-2` (the
...
| `docs/DESIGN.md`                | The design system — third edition, the consumer-banking layout|
