...
a modern neobank: a soft-grey ground, white cards raised by luminance alone, one sans
...
| Light is the default. Dark is the system preference or the explicit choice — charcoal `#121416` ground, `#1f2226` cards |
| Press is luminance. Nothing scales. Shadow only under what floats: the glass bar, the glass toast, the sheet            |
| Glass (`.glass`) is for what sits over content or on the ground — never for anything inside a card (Q58)                |
...
| `.seg`, `.seg__item`                   | The segmented pill; `aria-pressed` selects in the primary pill's colours; `--soft` inside a card, `--glass` on Zápis |
| `.glass`                               | The frosted surface: `--glass`, a `--glass-edge`, backdrop blur. Zápis's controls; never inside a card               |
| `.btn`                                 | Soft pill 40 px; `--primary` 48 inverted, `--card`, `--quiet`, `--danger`, `--sm`, `--lg`, `--block`                 |
| `.round`                               | A 40 px round glass button on the ground: close, back, settings                                                      |
...
The tab bar floats: a frosted pill (`--glass`, backdrop blur, `--elev-glass`)
absolutely positioned 24 px over the page's bottom edge — or 12 px past the
home indicator (`--tabbar-lift`). `.app:has(.tabbar)` sets `--page-end` and
`--toast-lift` so the last row and a confirmation both clear it. `/zapis`
deliberately has no tab bar — the keypad owns the bottom of the phone, and the
bar's centre disc is how you get there. In the dark, `/zapis` paints `--glow`
behind its glass. The bar is _liquid_ (Q60): a light-turning rim
(`--glass-shine` → `--glass-rim`), a top highlight, a `--glass-lens` pill that
springs to the current tab's slot, and — on Chromium only, gated on
`navigator.userAgentData` — an SVG displacement map in the backdrop filter
that bends the content under the rim, ahead of a 6 px blur. `TabBar.svelte`
draws the map on a canvas for the bar's size; Safari gets everything but the
bend.
