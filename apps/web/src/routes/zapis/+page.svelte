...
<script lang="ts">
	 ...
	 *
	 * Every control on this screen is glass: it sits on the ground, and in the
	 * dark the ground carries the glow — two soft lights the frost has
	 * something to blur against. The keys stay bare.
	 */
	import { liveQuery } from 'dexie';
	import { afterNavigate, goto, invalidateAll } from '$app/navigation';
	import { resolve } from '$app/paths';
	import { db } from '$lib/db/schema';
	import { createCategory, createTxn, deleteTxn, setActiveAccountId } from '$lib/db/repo';
	import {
		EMPTY,
		display,
		isSavable,
	...
	/** A bucket made from the picker is the bucket this entry goes to. */
...
<main class="screen">
	<div class="flow">
		<header class="head">
			...
			<div class="seg seg--glass" role="group" aria-label="Směr">
				<button
					type="button"
					class="seg__item"
...
<style>
	...
	.screen {
		...
		/* The lights behind the glass. `none` in the light theme. */
	...
	.payee:focus-visible {
		...
		box-shadow: 0 0 0 0.5px var(--signal);
	}
