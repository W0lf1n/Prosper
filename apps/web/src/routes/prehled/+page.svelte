<script lang="ts">
	...
	import { DAYS, PAYMENTS, capitalize, counted, plural } from '$lib/domain/czech';
	...
	/**
	 * Which bucket is open to its rows (Q63). One at a time, so the ranking
	 * stays a ranking; keyed with the month and the account, so turning the
	 * page closes it without an effect.
	 */
	let openBucket = $state<string | null>(null);
	const bucketKey = (id: string | null) => `${month}:${view}:${id ?? 'none'}`;
	function toggleBucket(id: string | null) {
		const key = bucketKey(id);
		openBucket = openBucket === key ? null : key;
	}
			...
			<section class="card buckets">
				...
				{:else}
					{#each summary.buckets as bucket (bucket.category?.id ?? 'none')}
						...
						{@const open = openBucket === bucketKey(bucket.category?.id ?? null)}
						<button
							type="button"
							class="bucket"
							class:bucket--open={open}
							aria-expanded={open}
							onclick={() => toggleBucket(bucket.category?.id ?? null)}
						>
							<span
								class="circle"
						...
						{#if open}
							<ul class="bucket__rows">
								{#each bucket.rows as row (row.id)}
									<li class="row row--short">
										<span class="row__body">
											<span class="row__title">{row.payee || row.note || 'bez popisu'}</span>
											<span class="row__sub">
												{capitalize(formatDayHeading(row.date))}{row.isOneOff ? ' · mimořádný' : ''}
											</span>
										</span>
										<span class="row__amount" class:bucket__refund={row.amount > 0}>
											{formatMoney(row.amount, { sign: 'always', code: currency })}
										</span>
									</li>
								{/each}
							</ul>
...
<style>
	...
	/* The chips of the bucket rail, without the circle, so the padding is
	   even. They were card-coloured pills until 2026-09-06 (Q61): in the dark
	   a graphite pill on charcoal was barely there, and the chosen one had
	   lost its fill to the cascade — `.btn--card` outranks `.btn--primary`. */
	.accounts .chip {
		padding: 0 14px;
	}
	...
	/* A bucket is the control that opens it (Q63): the whole line is the
	   target, and it presses by luminance like a row. */
	.bucket {
		display: flex;
		align-items: center;
		gap: var(--space-3);
	...
	.bucket--open .bucket__chevron {
		transform: rotate(180deg);
	}

	/* The rows, indented to the bucket's name. */
	.bucket__rows {
		list-style: none;
		margin: 0 0 var(--space-1);
		padding: 0 0 0 calc(40px + var(--space-3));
	}
