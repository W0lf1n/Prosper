...
describe('recurringCost', () => {
	...
	it('counts a shared payment at what it actually costs', () => {
		...
		// balance sees 30 000 on the 15th; the decision is made against 15 000.
		const total = recurringCost([
			schedule('hypo', { amount: minor(-30_000_00), shares: [share(15_000_00, 'Partner')] })
		]);

		const row = total.rows[0]!;
		expect(row.monthly).toBe(30_000_00);
		expect(row.reimbursed).toBe(15_000_00);
		expect(row.net).toBe(15_000_00);
		expect(row.yearly).toBe(180_000_00);
		expect(row.grossYearly).toBe(360_000_00);

		expect(total.monthly).toBe(30_000_00);
		expect(total.reimbursed).toBe(15_000_00);
		expect(total.net).toBe(15_000_00);
		expect(total.yearly).toBe(180_000_00);
	});
...
describe('netOfSchedule', () => {
	...
	it('subtracts the declared share', () => {
		expect(
			netOfSchedule(schedule('hypo', { amount: minor(-30_000_00), shares: [share(12_500_00)] }))
		).toBe(17_500_00);
	});
	...
	it('subtracts every payer of a split subscription — Q47', () => {
		expect(
			netOfSchedule(
				schedule('netflix', {
					amount: minor(-399_00),
					shares: [share(133_00, 'Alex'), share(133_00, 'Bea')]
				})
			)
		).toBe(133_00);
	});
...
describe('scheduleSharesOf — the legacy fallback', () => {
	it('synthesises the single share of a pre-v9 schedule', () => {
		const old: Record<string, unknown> = { ...schedule('hypo', { amount: minor(-30_000_00) }) };
		delete old.shares;
		old.owedAmount = 15_000_00;
		old.owedBy = 'Partner';

		expect(scheduleSharesOf(old as unknown as Schedule)).toEqual([
			{ id: 'legacy', who: 'Partner', amount: 15_000_00 }
		]);
		expect(netOfSchedule(old as unknown as Schedule)).toBe(15_000_00);
	});
...
describe('sharesForPosting', () => {
	it('copies the declared shares onto an ordinary posting', () => {
		...
		expect(sharesForPosting(netflix, minor(-399_00))).toEqual([
			{ who: 'Alex', amount: 133_00 },
			{ who: 'Bea', amount: 133_00 }
		]);
	});
	...
	it('never books back more than went out on an overridden amount', () => {
		...
		expect(sharesForPosting(netflix, minor(-200_00))).toEqual([
			{ who: 'Alex', amount: 133_00 },
			{ who: 'Bea', amount: 67_00 }
		]);
		// Confirmed at 100: only the first share, clamped, survives.
		expect(sharesForPosting(netflix, minor(-100_00))).toEqual([{ who: 'Alex', amount: 100_00 }]);
	});

	it('gives an incoming amount no shares at all', () => {
		const odd = schedule('odd', { shares: [share(133_00, 'Alex')] });
