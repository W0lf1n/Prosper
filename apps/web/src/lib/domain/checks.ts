...
export function checkDraft(draft: Draft, context: CheckContext): Finding[] {
	...
	// Nearly twice as much food was filed under BYDLENÍ, LIFESTYLE, DARY and
	...
	// "Netflix - 379" recorded as 74 Kč. "objednávka (2415 Bea)" as 5 008 Kč.
	...
	// "opak. obj" carried a five-figure sum in a single month.
	...
	// KVĚTEN cost sixty percent more than a usual month, because a five-figure
...
export interface BucketTotal {
	...
	oneOffTotal: Minor;
	count: number;
	/** The rows themselves, newest first — what a bucket opens to on Přehled. */
	rows: Txn[];
	/** Percent of the month's whole outflow — what the bar draws. */
	share: number;
	/**
	 * Percent of the month's *recurring* outflow. This is the honest one: a
	 * five-figure front door should not make the dumping-ground bucket look small.
	 */
...
function checkMonth(
	...
	// OSTATNÍ carried roughly a sixth of all outflow across eight months — the
	// second largest bucket after housing, and it explains nothing about where
	// the money went.
	...
	// Four of the eight months ran a deficit, and the spreadsheet never said so.
