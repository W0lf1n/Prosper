...
describe('misfiled category', () => {
	it('spots lunch filed under housing — the misfiled-food mistake', () => {
...
describe('unclear records', () => {
	...
	it('flags a vague description on a large amount — "opak. obj" at five figures', () => {
...
describe('one-off spending', () => {
	it('offers to mark a large expense — the five-figure front door', () => {
...
describe('refund recorded as income', () => {
	it('catches "Bea - bydlení plyn" landing in income', () => {
		const found = checkDraft(
			draft({ direction: 'in', amount: minor(125000), payee: 'Bea - bydlení plyn' }),
			context()
		);
	...
	it('leaves an actual salary alone', () => {
		const found = checkDraft(
			draft({ direction: 'in', amount: minor(4000000), payee: 'výplata' }),
			context()
		);
...
describe('summariseMonth()', () => {
	...
	it('measures OSTATNÍ against recurring spending, not against a one-off', () => {
		// Without this, a single five-figure front door makes the dumping ground
	...
	it('nets a refund against its bucket instead of calling it income', () => {
		// You paid 2 500 for gas and Bea gave back 1 250. The month earned
		...
		const summary = summariseMonth({
			...
			txns: [
				...
				txn('2026-08-20', 125000, { categoryId: 'cat-bydlení', payee: 'vrácení — Bea' })
			]
		});
	...
	it('still counts a real salary as income', () => {
		const summary = summariseMonth({
			...base,
			month: '2026-08',
			txns: [txn('2026-08-01', 5000000, { categoryId: 'cat-příjem', payee: 'výplata' })]
		});
	...
	it("carries each bucket's own rows, newest first", () => {
		const summary = summariseMonth({
			...base,
			month: '2026-08',
			txns: [
				txn('2026-08-01', -50000, { categoryId: 'cat-jídlo', payee: 'Lidl' }),
				txn('2026-08-14', -12000, { categoryId: 'cat-jídlo', payee: 'Albert' }),
				txn('2026-08-14', 3000, {
					categoryId: 'cat-jídlo',
					payee: 'vratka',
				...
				txn('2026-08-02', -400000, { categoryId: 'cat-bydlení' }),
				txn('2026-07-30', -9900, { categoryId: 'cat-jídlo', payee: 'minulý měsíc' })
			]
		});
		const food = summary.buckets.find((b) => b.category?.name === 'JÍDLO')!;
		expect(food.rows.map((t) => t.payee)).toEqual(['vratka', 'Albert', 'Lidl']);
		expect(food.rows.length).toBe(food.count);
	});
});
