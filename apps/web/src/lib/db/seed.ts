 ...
 *   SPOŘENÍ is new (Q24). In the spreadsheet "dlouhodobá investice" — the same
 *   sum every month — sat inside OSTATNÍ, which means the one
...
export function seedCategories(): CategorySeed[] {
	return [
		...
		// Last on purpose. OSTATNÍ took roughly a sixth of eight months' outflow —
		// the second biggest bucket in the book. Making it the least convenient chip is the
