...
describe('shares — one expense, several payers (Q47)', () => {
	it('settles one share without touching its neighbour', async () => {
		const netflix = await createTxn({
			...
			payee: 'Netflix',
			shares: [
				{ who: 'Alex', amount: 13300 as Minor },
				{ who: 'Bea', amount: 13300 as Minor }
			]
		});
		const alex = netflix.shares[0]!;

		const repayment = await settleReceivable(netflix.id, alex.id);

		expect(repayment?.amount).toBe(13300);
		expect(repayment?.payee).toBe('vrácení — Alex');
	...
	it('refuses to settle the same share twice', async () => {
		const txn = await createTxn({
			...
			amount: -39900 as Minor,
			payee: 'Netflix',
			shares: [{ who: 'Alex', amount: 13300 as Minor }]
		});
	...
	it('unsettle removes the inflow and reopens only that share', async () => {
		...
		const [alex, bea] = txn.shares;
		const repayAlex = await settleReceivable(txn.id, alex!.id);
		const repayBea = await settleReceivable(txn.id, bea!.id);

		await unsettleReceivable(txn.id, alex!.id);

		const after = await db.txns.get(txn.id);
		expect(after?.shares[0]?.settledByTxnId).toBeNull();
		expect(after?.shares[1]?.settledByTxnId).toBe(repayBea!.id);
		expect((await db.txns.get(repayAlex!.id))?.isDeleted).toBe(true);
	...
	it('confirmScheduled copies the declared shares onto the posted row, with fresh ids', async () => {
		...
		// Fresh ids: settling August's Alex must not implicate September's.
	...
	it('a settled legacy row is upgraded to the array shape on the way through', async () => {
		...
		expect(after?.shares).toEqual([
			{ id: 'legacy', who: 'Bea', amount: 125000, settledByTxnId: repayment!.id }
		]);
	});
});
