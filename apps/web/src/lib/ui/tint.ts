 ...
 * one place. Always, not only while tinted: the browser reads the meta whose
 * `media` matches the *system* scheme, so a phone in light mode with the app
 * set to dark used to get the light literal back the moment the tint went —
 * a pale strip of status bar over a charcoal screen (Q61). Called again
 * whenever the theme moves, from `applyTheme` and from the layout's media
 * listener, because the resolved ground moves with it.
 */
