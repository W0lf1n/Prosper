<script lang="ts">
	...
	interface Props {
		...
		/**
		 * What a new bucket starts as — the name typed into the picker's search
		 * and the direction of the entry it was typed on. Ignored on an
		 * existing bucket.
		 */
		draft?: { name?: string; isIncome?: boolean };
	...
	let { open, category, draft, oncreate, onpatch, onclose }: Props = $props();

	const SPEND_TYPES = (Object.keys(SPEND_TYPE_LABEL) as SpendType[]).map((value) => ({
		value,
		label: SPEND_TYPE_LABEL[value]
	}));
	...
	$effect(() => {
		...
		isIncome = category?.isIncome ?? draft?.isIncome ?? false;
